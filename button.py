...
def create_buttons(num_hor, num_ver, space_hor, space_ver, width, height, text, clicked, coord, cable_dict):
    timer = []
    buttons_list = []
    text_list = []
    color_list = []
    ...
    for i in range(num_hor * num_ver):
        text_list.append(str((cable_dict[text[i]])[0]))
    for i in range(num_hor * num_ver):
        timer.append(0)
    return [buttons_list, text_list, clicked, color_list, timer, text]
